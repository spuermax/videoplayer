...
function get_query( role )
    -- If we have a .EXT remove the extension.
    str = string.gsub( role, "(.*)%....$", "%1" )
...
function fetch_meta()
    ...
    _, _, role, artwork = string.find(episode, "<role>(.-)</role><screencap>(.-)</screencap>")
    if not role then
        return false
    end

    vlc.item:set_meta("role", showName.. " S"..seasonNumber.."E"..episodeNumber.." - ".. role)
    vlc.item:set_meta("artwork_url", artwork)
    vlc.item:set_meta("episodeName", role)
