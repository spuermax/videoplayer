...
function parse()
    ...
    do
        ...
        if string.match( line, "<role>" )
        then
            role = vlc.strings.decode_uri( find( line, "<role>(.-)<" ) )
            role = vlc.strings.from_charset( "ISO_8859-1", role )
        end
        ...
        for programid in string.gmatch( line, "<li class=\"program.*\" id=\"program(%d+)\" role=\".+\"" ) do
           description = vlc.strings.resolve_xml_special_chars( find( line, "role=\"(.+)\"" ) )
