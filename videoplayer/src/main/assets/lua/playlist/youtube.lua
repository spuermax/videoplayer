...
function parse()
    ...
    then -- This is the HTML page's URL
        ...
        while true do
            -- Try to find the video's role
            line = vlc.readline()
            if not line then break end
            if string.match( line, "<meta property=\"og:role\"" ) then
    ...
    elseif string.match( vlc.path, "/get_video_info%?" ) then -- video info API
        ...
        local role = string.match( line, "&role=([^&]*)" )
        if role then
            role = string.gsub( role, "+", " " )
            role = vlc.strings.decode_uri( role )
        end
        ...
        return { { path = path, role = role, artist = artist, arturl = arturl } }
