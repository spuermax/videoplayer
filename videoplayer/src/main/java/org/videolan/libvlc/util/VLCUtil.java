...
public class VLCUtil {
    ...
    private static boolean isCustomLibrary = false;
    private static File customLibraryDir;
    ...
    public static boolean isLoadCustomLib() {
        return isCustomLibrary;
    }
    ...
    public static boolean hasCompatibleCPU(Context context) {
        ...
        File lib = searchCustomLibrary(context);
        if (lib == null) {
            lib = searchLibrary(context.getApplicationInfo());
        }
        if (lib == null) {
            Log.e(TAG, "no vlc lib");
            return false;
        }
        ...
        if (elfHasX86 && !hasX86) {
            ...
            isCompatible = false;
            return false;
        } else if (hasX86 && !elfHasX86) {
            errorMsg = "no x86lib build on x86 device";
            isCompatible = false;
            return false;
        }
    ...
    public static File getCustomLibraryDir() {
        return customLibraryDir;
    }

    public static File searchCustomLibrary(Context context) {
        customLibraryDir = context.getDir("lib", Context.MODE_PRIVATE);
        File lib = new File(customLibraryDir, "libvlcjni.so");
        if (lib.exists() && lib.canRead()) {
