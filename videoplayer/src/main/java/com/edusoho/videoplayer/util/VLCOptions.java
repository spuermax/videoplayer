...
public class VLCOptions {
    ...
    public static ArrayList<String> getLibOptions(Context context) {
        ...
        if (!AndroidUtil.isKitKatOrLater()) {
            chroma = context.getResources().getString(R.string.chroma_format_yv12);
        }
        ...
        int networkCaching = pref.getInt("network_caching_value", NETWORK_CACHE);
        if (networkCaching > 60000)
            networkCaching = 60000;
        else if (networkCaching < 0)
            networkCaching = NETWORK_CACHE;
        ...
        if (networkCaching > 0)
            ...
            options.add("--live-caching=" + networkCaching);
