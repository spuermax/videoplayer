...
public class BitmapUtil {
    ...
    public static Bitmap getPictureFromCache(Context context, MediaWrapper media) {
        // mPicture is not null only if passed through
        // the ctor which is deprecated by now.
        Bitmap b = media.getPicture();
        if (b == null) {
            final BitmapCache cache = BitmapCache.getInstance(context);
    ...
    public static Bitmap fetchPicture(Context context, MediaWrapper media) {
        final BitmapCache cache = BitmapCache.getInstance(context);

        Bitmap picture = readCoverBitmap(context, media.getArtworkURL());
        if (picture == null) {
    ...
    public static Bitmap getPicture(Context context, MediaWrapper media) {
        final Bitmap picture = getPictureFromCache(context, media);
        if (picture != null)
            return picture;
        else
            return fetchPicture(context, media);
    }
